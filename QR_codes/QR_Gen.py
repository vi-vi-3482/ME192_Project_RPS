...
for i, v in enumerate(grid_corners):
    ...
    qrcode.save("grid_corners_%d.png" % i, scale=10, dark="darkblue")


for i, v in enumerate(rps_blocks):
    qrcode = segno.make_qr(str(v))
    qrcode.save("rps_block_%d.png" % i, scale=10, dark="darkred")

for i, v in enumerate(rps_cards):
    qrcode = segno.make_qr(str(v))
    qrcode.save("rps_card_%d.png" % i, scale=10, dark="darkgreen")
